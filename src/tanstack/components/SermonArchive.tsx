...
const SermonArchive: FC = () => {
  const context = useRouteContext({ from: "/$all" });
  const { allSermonData, allSeriesData, allPreachersData, paths } = context;
  if (!allSermonData || !allSeriesData || !allPreachersData)
    throw new Error("Missing data");

  const [showFilters, setShowFilters] = useState(false);

  const { state, setters, hasFilters, resetFilters, filteredData } =
    useFilters(allSermonData);
